...
F = FeedbackRegister.from_file('C162_stored.json')
F.fn.compile()

def cmprtrivium_v2(key, iv, keystream_length):
    initrounds = 100
    initstate_iv = str2list(iv)
    initstate = str2list(key)
    if (len(key) != 80):
        print("Key must be 80 bits.")
        quit()
    if (len(iv) != 80):
        print("IV must be 80 bits.")
        quit()
    ...
    F.seed([1,1] + initstate_iv + initstate)
...
iv = generate_random_binary(80)
j = 1
for line in lines:
    key = line.strip()
    keystream = cmprtrivium_v2(key, iv, 128)
